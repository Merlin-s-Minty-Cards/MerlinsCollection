...
import { ShoppingBag, Plus, X, Check, Banknote, CreditCard } from 'lucide-react'
...
export default function AdminBuyPage() {
  ...
  const addItem = async () => {
    ...
    try {
      ...
      setForm({ name: '', condition: 'NM', buy_price: '', market_value: '', set_name: '', location: 'toploader' })
    } catch (err) {
          ...
          <div className="vault-panel rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <label>
                ...
                <input type="number" step="0.01" value={form.buy_price} onChange={(e) => setForm((f) => ({ ...f, buy_price: e.target.value }))} className="vault-field w-full mt-1 px-2.5 py-1.5 rounded-lg text-xs" placeholder="0.00" />
              </label>
              <label>
                <span className="text-[11px] text-pine-400">Market Value ($)</span>
                <input type="number" step="0.01" value={form.market_value} onChange={(e) => setForm((f) => ({ ...f, market_value: e.target.value }))} className="vault-field w-full mt-1 px-2.5 py-1.5 rounded-lg text-xs" placeholder="0.00" />
              </label>
            </div>
            <label>
          ...
          <div className="vault-panel rounded-xl p-4 space-y-3">
            ...
            <label className="block">
              ...
              <div className="flex gap-2 mt-1">
                {[{ value: 'cash', icon: Banknote, label: 'Cash' }, { value: 'card', icon: CreditCard, label: 'Card' }].map(({ value, icon: Icon, label }) => (
