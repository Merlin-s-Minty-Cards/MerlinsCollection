...
| **OQ-4** | ~~D4 says "ask before deleting" anything ambiguous. **Delete `graded_price` and `item_price_point` in the wipe?**~~ **RESOLVED — yes, delete both.** | Owner approved (2026-07-26). Both are keyed to identifiers (`card_id`, `item_id`) that the reseed/re-import regenerates, so keeping them would leave permanently unreachable rows keyed to dead pokemontcg ids. Both are re-derivable (sheet Sticker/Current Market; the daily sealed snapshot). |
| **OQ-5** | ~~`finalize_import(committed=True)` will delete `expense`/`debt`/`payout` because the finance tabs are out of scope and nothing recreates them.~~ **RESOLVED — the premise was wrong; there is no data-loss risk.** | Two facts dissolve it. (1) `run_import` imports the **whole workbook** via `_TAB_IMPORTERS` — shows, cash, buying guidelines, and every finance tab — and only then calls `finalize_import(committed=True)`, so every swept entity is recreated in the new generation. (2) The **completeness precondition** (`require_complete=True`, [spreadsheet_import.py:1093](../../backend/src/merlins_collection/services/spreadsheet_import.py#L1093)) already refuses to touch the DB unless *every* expected tab CSV is present — an intentional subset export raises **before any write**. All 23 workbook tabs have now been exported to `data/spreadsheet/csv/`, so the precondition passes and safe-replace works as designed. No change to `_IMPORT_OWNED_ENTITIES` is needed or wanted. |
| **OQ-8** | **Slabs: schema now, import later.** Owner decision (2026-07-26): the graded-slab *data model* is in scope, but the 17 slab rows must NOT be imported until they can be scanned in via PSA cert lookup. | Keep `GradedInventoryItem`, `GradingCompany`, cert fields, and `PricePoint(kind="graded", ...)` in the model and keep them exercised by unit tests. Hold the Slabs **tab import** back using the existing per-tab truncation knob: `run_import(..., allow_empty={"Slabs"})`, so a zero-row Slabs import is an explicit acknowledgement rather than a run failure. This also defers Q2 (PriceCharting key) and Q3 (PSA token) out of the critical path entirely — neither is needed to ship the rest. |
