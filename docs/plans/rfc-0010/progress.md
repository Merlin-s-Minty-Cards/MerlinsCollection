...
T11 gave the ledger a correction path — a void, never a delete, with one
countability predicate every aggregate calls; T12 withdrew PSA, removed the
...
| T11 | Transaction void | **DONE** | `eb6e15a` | **Sales only; a purchase gets a 400** (open owner question decided unattended — see Decisions). New `services/ledger.py` holds `is_countable` **and `countable(rows)`**, the sugar every reader actually calls so none of them can spell the filter differently. `GET /admin/transactions` is the one deliberate non-filterer and says so at the call site. `Transaction` gains `voided_at`/`voided_by`/`void_reason`; `ShowAnalyticsSnapshot` gains **`stale`**. Repo gains `get_transaction`, `list_transactions_by_batch`, `reverse_sales` (atomic, all legs or none) and `reverse_sale` as a one-line delegate. **FOUR routes, not two** — `/transactions/{id}/void|restore` plus `/transactions/batch/{batch_id}/void|restore`, because T10's `batch_id` is the key that selects a whole transaction. `ConfirmDialog` gained `children` + `confirmDisabled`; new shared `VoidDialog`. `formatTimestamp` (T8) now has callers |
| T12 | Slabs: PSA out, price at intake | **DONE** | `b5b41b9` | `CertInput` kept Enter-advances and `

` stripping — both now called out in its docstring as the reason removing the button is safe; its six tests are untouched. `refresh_graded_prices` gained **`only_item_ids`**, applied inside `_graded_candidates` so a filtered-out slab costs zero calls. **`confirm_buy_session` now returns `item_ids`** (additive; `/admin/buy` ignores it) — without it the frontend cannot scope the refresh. Pricing runs after the commit, un-awaited, on its own status line. RFC 0009 T2/T5 are **WON'T DO** in that plan's progress file |
| T13 | Grouped navigation | **DONE** | `8ba6ac9` | Every route path unchanged, `/admin/outgoing` included. **Groups default to OPEN** and the active group is forced open regardless of what was saved (see Decisions) — that is what kept the five pre-existing `AdminShell` tests green. Badge rolls onto a group header **only while that group is shut**. `navItems` is gone, replaced by `topLevel` + `navGroups` + an explicit **`mobileItems`**; nothing outside the file imported it. The test file's `usePathname` mock is now **mutable** (`mockPathname`) so a test can render at another route |
...
| T-FINAL | Verification + PR | **DONE** | `036e5f2` | **Verified at `d5332bb`**, the last commit touching code or CLAUDE.md; `036e5f2` is this row plus plan-doc prose and changes nothing the suites read (the 24 doc guards were re-run green at the tip to confirm). **Backend 1671 passed / 0 failed; frontend 810 passed / 0 failed (87 files); MCP 98 passed / 0 failed (7 files); `next build` exit 0; both linters clean.** **The doc's "known pre-existing `ChatPanel` failure" is STALE and was corrected in the doc** — it was fixed 2026-08-11 and passes 12/12 both alone and under full parallel load, so **the bar here is 0 failed, not "6–7 expected"**. Its §5 boot command was also broken two ways (see Decisions). Leak sweep clean across all **232** branch commits; `backend/.env` has never been committed. **Three manual checks still need the owner** — T11's void end-to-end, T12's wedge scan, T13's sidebar |
...
| ~~Does voiding a PURCHASE need to work in the first cut?~~ | **CLOSED 2026-08-12 — decided unattended, at the owner's instruction, and built the honest small way: SALES ONLY.** A purchase returns a clear `400 "A purchase cannot be voided yet — remove the item from inventory instead"`, and the UI does not offer the action at all rather than offering one that always fails. Consequence, recorded because it is real: **a mistaken BUY still has no correction path, and a TRADE cannot be voided either** (its legs share a `batch_id` but one of them is a PURCHASE). Both are in follow-ups with what a proper fix would need | — |
...
| 2026-08-12 | T11 | **The open owner question was decided unattended: voiding a PURCHASE is REFUSED with a 400, sales only.** The Blocked row is closed above | The instruction was to pick the reading that is safer for money and for the ledger. Voiding a sale is well defined — the card comes back. Voiding a purchase means removing an item that may since have been sold, traded, re-priced or consigned, and a void that leaves a phantom item in stock is worse than no void at all. The task doc calls this "the honest small version" itself. The UI does not render the action on a purchase group rather than rendering one that always 400s — an affordance that always fails is worse than none |
| 2026-08-12 | T11 | **There are FOUR routes, not the doc's two: a batch void and a batch restore keyed on T10's `batch_id`** | The doc requires the primary action to be *"Void this whole transaction (5 cards)"*, and the only way to do that with per-row routes is five POSTs that can half-succeed — the exact partial-write class T0 was created to fix, on the same money path. `reverse_sales` issues ONE `transact_write_items` for every leg, and every leg is guarded before anything is written, so a batch containing one card that has moved on since voids nothing at all. `reverse_sale` is a one-line delegate, so there is still a single reversal implementation |
| 2026-08-12 | T11 | **A batch of more than 50 legs is REFUSED with a 422 telling the operator to void them one at a time, rather than chunked** | DynamoDB caps a transaction at 100 actions and a reversal spends two per leg. Chunking is the obvious alternative and it silently reintroduces partial write — the thing the atomic call exists to prevent. Refusing is a dead end for a hypothetical 51-card session; a half-voided 51-card session is a wrong set of books. Filed in follow-ups with what a real fix would need |
| 2026-08-12 | T11 | **`voided_by` stores `email or username or sub` — one value with a fallback chain, not two fields, and never a client-supplied one** | The doc says "pick one and document it — do not store both". An audit line is read by a human, so the email is the right first choice; `sub` is the last resort because it is the one claim that always exists, so the field is never null on a row whose whole purpose is accountability. Under the test token that resolves to `merlin`, which is a named assertion |
| 2026-08-12 | T11 | **`attribute_not_exists(voided_at)` is NOT the "not voided yet" guard, and using it alone would have made the double-void check silently useless** | `put_transaction` writes every model field including the `None`s, so a row written since T11 carries `voided_at` as a DynamoDB **NULL — which exists**. Probed against moto before relying on it: the guard is `attribute_not_exists(voided_at) OR attribute_type(voided_at, "NULL")`, and the restore side checks `attribute_type(voided_at, "S")` rather than mere presence. The route's own 409 pre-check would have hidden the defect in every test; only a concurrent second void would have found it |
| 2026-08-12 | T11 | **The void writes ONE timeline event per transaction, keyed `<txn_id>#void`, and a restore RE-PUTS it as `void_restored` rather than appending a second event** | Two hard constraints collide here. The original sale event is keyed `TIMELINE#<date>#<txn_id>`, so a void on the same day — the common case — would have OVERWRITTEN the sale, which is the one thing the doc forbids by name; hence the suffix. And `get_timeline_events` sorts by DATE only, so a void and a restore on the same day have no defined order and `#restore` sorts before `#void` in the key — two events could not be read back as a state. One event that always states the current state has neither problem, and a re-void overwrites it back |
| 2026-08-12 | T11 | **`repo.get_transaction` walks month partitions NEWEST FIRST and stops at the first hit, rather than taking the date from the client** | The SK embeds the transaction's date, which the caller of a void does not have to know, and asking the client for it makes the key partly client-supplied on a money-reversing route. Newest-first makes the ordinary case — correcting a mistake made minutes ago — a single Query, bounded at ~37 for an unknown id. `list_transactions_by_batch` shares the walk and stops at the first month with any leg, which is exact because one confirm writes one `txn_date` for every row it creates |
| 2026-08-12 | T11 | **The archive's group total sums only the COUNTABLE legs; a fully-voided group renders what it withdrew, struck through** | A partially-voided group has to show what it still contributes, or the archive line and the day's tiles disagree on screen — the two-sets-of-books failure in miniature. A fully-voided group showing `$0.00` would be technically consistent and useless, so it shows the original figure with a line through it. `TransactionGroups` also exports `isCountable`, whose docstring says plainly that it is a RENDERING rule and that the server owns every figure a decision is made on |
| 2026-08-12 | T11 | **One RED test passed against unfixed code for the WRONG REASON and was hardened: `unknown txn_id → 404`** | FastAPI 404s a route that does not exist, so the assertion was satisfied by the absence of the entire feature. It now asserts `detail == "Transaction not found"`. This is the failure mode the RED gate exists to catch, and counting failures would have missed it — 23 failed, 1 passed, and the 1 was the one that mattered. Two other RED tests failed for defects of MINE (a `status="traded"` that is not in `ItemStatus`, and a serializer warning from assigning an ISO string where a `datetime` belongs); both were fixed and re-run RED |
| 2026-08-12 | T11 | **`ShowAnalyticsSnapshot.stale` is RENDERED on `/admin/analytics` — the show list row and the detail view — though the doc's RED list is backend-only** | The doc's rule is *"never silently rewrite one, and never silently serve a stale one"*. A flag no page shows is a silent serve. Same call as T16's and T17's coverage panels. The detail banner is `role="alert"` beside the regenerate button, which is the only place the operator can act on it |
| 2026-08-12 | T12 | **`confirm_buy_session` now returns `item_ids` — a shared money endpoint changed to serve one page** | The doc requires the post-commit refresh to be *"scoped to the created item ids"* and the response carried only a count, so the alternatives were a second pricing path (banned by the doc), an unscoped refresh (which spends the whole 50-lookup day on the shelf — the exact thing the scope exists to prevent), or re-deriving the ids by querying inventory for the certs just typed. Additive and ignored by `/admin/buy`, with its own RED test |
| 2026-08-12 | T12 | **`only_item_ids` filters inside `_graded_candidates`, not in the pricing loop** | The doc's requirement is that a filtered-out slab costs ZERO provider calls, and the dedupe by `(card_id, company, grade)` happens in that same function — filtering later would let an excluded slab consume the dedupe slot of an included one at the same grade, so the included slab would be silently skipped. Its own named test asserts the recording fake was never called |
| 2026-08-12 | T12 | **Three pre-existing tests were DELETED, deliberately** — "arms the cert field for a scan" and the two "disabled, naming PSA approval" tests | Their subjects no longer exist, so there was nothing to rewrite them into. Their replacements pin the ABSENCE of those affordances, and `CertInput.test.tsx`'s six tests — the ones that make removing the scan button safe — are untouched. A comment at the deletion site says which tests went and why, so the next reader does not read the gap as an oversight. Same shape as T1's `min="0"` and T2's `active` rewrites, one step further |
| 2026-08-12 | T12 | **The pricing call is fired WITHOUT `await` from inside the commit's `try`** | A pricing failure must never reach the commit's `catch`, which sets *"Commit failed before confirming… Nothing was created"* — a sentence that would be flatly false after a successful confirm. `priceBatch` owns its error handling end to end and writes to a second state. The commit's success message is set before the call, so it lands first and unconditionally, which is the doc's own wording |
| 2026-08-12 | T12 | **Two of my seven frontend RED tests passed against unfixed code.** One ("keeps Manual entry as the one intake toggle") is a genuine regression guard and is kept as one; the other ("the commit message survives a failed refresh") passed only because nothing priced at all, and was hardened to also assert the refresh was attempted | Same lesson as T11's 404 above and T5's row-count tests: read WHY each test failed. A test that passes because the feature is absent is indistinguishable, in a summary line, from one that passes because the feature works |
| 2026-08-12 | T13 | **Groups default to OPEN, and the group holding the active route is forced open regardless of what was saved** | The doc specifies constraint 4 (active group starts expanded) but not the default. Defaulting to shut would have made every destination unreachable on a first visit and broken five pre-existing `AdminShell` tests that reach Triage, Shows and Show Prep from `/admin/inventory` — which is the honest signal that shut-by-default is a regression, not a preference. The cost is a taller sidebar; the nav has scrolled since RFC 0008 §F2 |
| 2026-08-12 | T13 | **The rolled-up badge appears ONLY while the group is shut** — the doc's open question, answered the less-noisy way | With the group open the count is on the Triage row itself, and the same number twice in a column of chrome is what trains the eye to stop reading it. The rule the badge exists to serve is *"a count nobody can see"*, and an open group's count is visible |
| 2026-08-12 | T13 | **`navItems` is deleted, not reshaped — `topLevel`, `navGroups` and a separate `mobileItems`** | The doc asks for the mobile nav to stop being a `.slice()`, and the reason is now demonstrable rather than theoretical: flattening the three groups and taking five yields **Trade** where Slabs used to be, because Trade precedes Slabs inside "At the show". Grepped for other importers first, as the doc's GREEN gate requires — there are none |
...
**Re-measured after T11, T12 and T13 at `8ba6ac9`** — a fresh run, not a row
above carried forward. All three landed in one session, so this is one
measurement across three feature commits rather than three:

| Suite | Count | Time | State |
|---|---|---|---|
| Backend | **1647 passed / 0 failed** | 2m38s | green — 1609 + 38 new (30 T11, 8 T12) |
| Frontend | **810 passed / 0 failed** (87 files) | ~33s | green — 778 + 32 new (12 T11, 11 T12, 9 T13) |
| MCP | **98 passed / 7 files** | 1.0s | green — re-run this time, see below |
| Narrow T11 selection (6 backend files) | 157 passed / 0 failed | ~16s | green — **127 of them pre-existing**, on the sale, timeline and analytics paths the void reaches into |
| Narrow T11 selection (3 frontend files) | 35 passed / 0 failed | ~5s | green |
| Narrow T12 selection (3 backend files) | 191 passed / 0 failed | ~30s | green — **184 of them pre-existing**, which is what proves `only_item_ids` did not disturb the nightly graded run |
| Narrow T12 selection (5 frontend files) | 50 passed / 0 failed | ~4s | green |
| Narrow T13 selection (1 frontend file) | 18 passed / 0 failed | ~1.5s | green — **9 of them pre-existing**, which is what proves the restructure dropped no destination |
| `ruff check backend/src` | — | ~3s | clean |
| `npm run lint --workspace=frontend` | — | ~5s | clean (the one pre-existing `<img>` warning in `CardDetailModal`, and only that one) |
| `npm run build --workspace=frontend` | — | ~16s | exit 0 |

**MCP was re-run this time**, unlike the last four sign-offs. Nothing in
`mcp-server/` reads a transaction, a slab price or the admin sidebar, so it was
not expected to move — but T11 changed a **model** (`Transaction`) rather than
only a router, and "no MCP file was touched" stops being a sufficient argument
the moment a shared shape changes. It is unchanged at 98.

**Not verified here, and each needs the owner — three manual checks, all of the
kind jsdom cannot fake:**

1. **T11's void, end to end.** Sell a card; confirm it leaves inventory and
   appears in the day's total. Void it with a reason. Confirm: the card is back
   as `available`, the day's total drops by that amount, the History row is
   struck through with the reason, and the item's timeline shows **both** the
   sale and the void. Open that show's analytics and confirm it now says *"out
   of date"*; regenerate and confirm the number moved. Then restore it and
   confirm everything goes back.
2. **T12's wedge scanner.** Open `/admin/slabs`, press Manual entry, and scan a
   real cert into the plain cert field. Confirm the digits land intact and focus
   advances to the card name. That is the behaviour the whole of Part 2 bets on,
   and the suite can only pin its two halves separately.
3. **T13's sidebar.** Click through all sixteen tabs, collapse the sidebar and
   confirm it is still navigable and the Triage count is still visible, reload
