...
| **T15** | Unified deal page, three modes | **DONE** | — | RFC Part 2. `frontend/app/(admin)/admin/trade/page.tsx` rewritten as `DealPage`, driven entirely by `lib/deal-session.ts` (`sessionApiFor(mode, api)` — the ONE place that picks `/purchases`, `/sales` or `/trades`). Mode read from `?mode=`, default `trade`, `router.replace` on toggle; switching mode with a non-empty session shows a discard `ConfirmDialog`, empty switches silently. Hidden columns are not rendered (`session.supports.incoming/outgoing` gates JSX, not CSS). New `DealStagedColumn` (Coming In / Going Out, reused twice) and `DealSummary` (cash components, balance hero — `font-mono`, net not summed-magnitude — profit, basis mode, date, Confirm) extracted to keep the page file to composition/state (435 lines). Customer view removes cost basis and profit from the DOM (conditional render, not `display:none`). 15 tests pass (`lib/__tests__/deal-session.test.ts` + the page test, which supersedes the old trade-only file); lint and `tsc --noEmit` clean. **Known gaps, deliberately out of scope for this task:** (1) editing a staged Going Out value is local-only — there is no `updateOutgoing` in the `DealSessionApi` interface the task doc specified, so an edited value is never re-sent to the session before confirm; the committed price is whatever `addOutgoing` originally posted. (2) "Split" cost-basis mode is offered in the UI (via the existing `trade-basis.ts`) but computed identically to Transfer, since T14's `IncomingLeg` carries no `market_value` to split against. Both are candidates for a follow-up task. |
