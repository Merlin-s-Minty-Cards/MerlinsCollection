...
**Last updated:** 2026-08-09 (**T6 DONE — `65ecece`.** Slabs can now be priced per
grade, and `/admin/slabs` lists graded stock with an honest "not priced" state.
T0/T1/T3/T4 also DONE; T2 and T5 remain DEFERRED behind PSA account approval and
block nothing. **T7 is the next task** — nothing calls `attach_price` yet)
...
| T7 | Nightly sync + refresh fix | NOT STARTED | — | **UNBLOCKED — this is the next task.** T6 shipped the provider, the storage and the list; T7 wires the job that walks the shelf. Budget is **50 slabs a night, not 100** (2 credits each), the quota counter is **per-process and in-memory** (follow-ups T6 row 2 — T7 should decide whether that moves to DynamoDB), and `MinuteWindow` already paces at 60/min so a loop does not need its own sleep |
