...
|---|---|---|
| **Get the PSA account approved for the public API** | Every call returns `403 {"Message":"Access to this API is limited to approved customers."}`. The key is valid and the header format is confirmed correct — **this is an entitlement, not a bug, and no code change can work around it.** Contact `collectors-apis@collectors.com` (the address PSA's own error body gives) or file PSA's public-API access request | **T2**, and the PSA half of T0 |
