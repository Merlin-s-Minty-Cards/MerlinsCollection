...
> # ⚠️ SUPERSEDED 2026-08-08 — and AMENDED, so two instructions are now gone
>
> **Build this instead:**
...
> **Those tasks are COMPLETE** (`b9a9798`, `170eb09`); T3 is `DONE`.
>
> The two errors, both of which would have cost you a cycle:
>
...
>    the item. **Validation belongs in `add_buy_item`, and that is where it shipped.**
>    The `BuySessionItem` section below is kept as the record of what was originally
>    specified, marked in place — **the model was left untouched.**
> 2. **"`cert_verified_at is None` → `cert_lookup_failed`" was REVERSED, and the rule
>    has now been DELETED from the body below** rather than left to be read and
>    followed. With manual entry the primary path it would flag *every* slab and turn
>    Triage into noise. Flag only on a missing `card_id`, which
>    `_review_reason_for_buy` already does as `no_catalog_link`.
>
...
## `BuySessionItem` changes — ⛔ NOT DONE, and deliberately so

> **This whole section was wrong and was not executed.** `BuySessionItem` is dead
> code: nothing constructs it, nothing depends on it, and `add_buy_item` takes a
> bare `dict[str, Any]`. Adding fields to it would have validated **nothing** while
> looking like validation, which is worse than no validation at all. **The model was
> left exactly as found.** The requirements below were satisfied in `add_buy_item`
> instead — see the section after it. Kept as the record of what was specified.
...
## Where the validation actually shipped

In `add_buy_item`, against the request body, returning **422** with the missing field
names — and the previously-staged items survive the rejection, which is the point:

```python
#: Fields a graded item cannot be staged without.
...
if kind == "graded":
    ...
    if missing:
        raise HTTPException(422, detail=f"graded items require: {', '.join(missing)}")
```

`cert_number` is required because **without one it is not a slab, it is just a normal
card** (owner, 2026-08-08) — and it is the key of T1's `CERT#` pointer row, so there
is nowhere to file the item without it.
...
**Review flagging for slabs — REVERSED 2026-08-08. A slab gets NO special rule.**

This section originally added: *"PSA never verified it (`cert_verified_at is None`) →
`cert_lookup_failed`"*. **That rule has been deleted, not merely deprecated, because
a doc that still contains it will eventually be followed.**

It assumed PSA verification was normal and its absence exceptional. **Manual-first
intake inverts that**, so the rule would flag *every* slab and turn Triage into
noise — which is how a review queue becomes something people stop reading.
`cert_lookup_failed` means *automation tried and failed*; a human deliberately typing
a slab in is the opposite. The codebase already holds this principle: the
`reviewed_at` guard exists so automation cannot re-flag what a human has passed
(CLAUDE.md, Triage).

**So `_review_reason_for_buy` (line 28) is reused UNCHANGED**, and the graded path
gets the same treatment as the raw one:

- No `card_id` → `no_catalog_link`, as today. It then also surfaces in Triage's
  derived `missing_card_id` chip — that is correct, not a bug; items routinely
  qualify under several reasons at once.
- A catalog-matched slab is **not flagged at all**, and
  `test_catalog_matched_slab_is_not_flagged_for_review` is the guard on that.

**Consequence for T4, and it is binding:** the frontend must **never send
`manual_entry`**. It is the one field that would re-create the flag-everything
behaviour from the other side. `cert_lookup_failed` keeps its literal meaning and
returns to use when T2 does.

This is a task that **removes** work rather than adding it.
...
7. ~~`cert_verified_at=None` → `needs_review` true with reason
   `cert_lookup_failed`.~~ **DELETED 2026-08-08 — do not write this test.** It
   asserts the reversed rule; writing it would drive the rule back into the code.
8. `card_id=None` → `needs_review` true; reason names the missing catalog link.
9. A catalog-matched slab is **not** flagged — `needs_review` false and
   `review_reason` `None`, *regardless of `cert_verified_at`*. This is the core of
   the manual-first pivot and the direct replacement for #7.
...
> **Answered 2026-08-08: no `_serialize` change was needed.** `_serialize` **recurses
> into lists**, so the JSON number inside the session's item list is already coerced
> `float` → `Decimal` (via `str()`, so the value round-trips exactly) on write. A
> JSON `buy_price: 900.50` reads back as `Decimal("900.50")`, not `900.4999…`, and
> `grade: 9.5` as `Decimal("9.5")`. The landmine is real elsewhere; **it does not
> bite this path** — but it was proved rather than assumed, which is why the tests
> post numbers and not strings.
