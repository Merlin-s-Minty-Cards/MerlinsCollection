...
4. **On PASS:** the change is done — continue the roadmap (typically `test-qa`, then `doc-writer`, then `pull-request`). Carry the Judge's non-blocking items forward as recorded follow-ups; do **not** patch-and-re-review them in another round.
