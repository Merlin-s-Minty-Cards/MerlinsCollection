...
- **Read-only except for one file:** `claude-progress.txt` is the only file created or modified here. Application code, tests, configs, documentation and dependencies are read and recorded exactly as found.
- Every claim in `claude-progress.txt` comes from something actually observed — a file read, a command run.
- If a previous `claude-progress.txt` exists, read it first, carry forward still-relevant open items, and note that a new baseline superseded the old one.
