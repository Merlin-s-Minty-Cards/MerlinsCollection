...
- **A PR describes committed history — nothing else exists to describe.** If the scope includes uncommitted work (step 2), that's not a reason to exclude it, but it IS a blocker: no PR can be opened until it's committed (and normally pushed). Say so first, plainly, before drafting anything — never bury it as a footnote inside an otherwise-polished, seemingly-ready body. A confident deliverable that can't actually be submitted is worse than no deliverable.
...
2. **Establish the diff scope.** Compare the current branch against the main branch (`git log main..HEAD`, `git diff main...HEAD --stat`, then the full diff for files needing close reading). Include uncommitted work only if the user says it belongs in the PR — if so, this is where the committed-history constraint above fires.
